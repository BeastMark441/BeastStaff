...
public class TelegramIntegration {
    ...
    public void notifyStatusChange(Player player, String oldStatus, String newStatus) {
        ...
        if (adminOnly && !player.hasPermission("beaststaff.admin")) return;
        
        plugin.getLogger().info("Отправка уведомления об изменении статуса для " + player.getName());
        
        String message = plugin.getMessageManager().getMessage("telegram-status-change", 
                "player", player.getName(),
                "old_status", oldStatus != null ? oldStatus : plugin.getMessageManager().getMessage("status-not-set"),
    ...
    public void notifyPlayerJoin(Player player) {
        ...
        String message = plugin.getMessageManager().getMessage("telegram-player-join",
                "player", player.getName(),
    ...
    public void notifyPlayerQuit(Player player) {
        ...
        String message = plugin.getMessageManager().getMessage("telegram-player-quit",
                "player", player.getName(),
                "rank", getPlayerRank(player));
    ...
    private String getPlayerRank(Player player) {
        if (plugin.getStaffManager().isStaffMember(player.getUniqueId())) {
            StaffManager.StaffMember member = plugin.getStaffManager().getStaffMember(player.getUniqueId());
            return member != null ? member.getRank() : plugin.getMessageManager().getMessage("status-unknown");
        }
    ...
    public void sendStatusCommand(Player player, String status) {
        ...
        String message = plugin.getMessageManager().getMessage("telegram-status-command",
                "player", player.getName(),
                "status", status);
    ...
    public void sendStatsCommand(Player player, int days) {
        ...
        Map<String, Long> stats = plugin.getTimeTrackingManager().getPlayerStats(player.getUniqueId(), days);
        long totalTime = stats.getOrDefault("total", 0L);
        
        String message = plugin.getMessageManager().getMessage("telegram-stats-command",
    ...
    public String handleCommand(String telegramId, String command, String[] args) {
        if (!isEnabled()) {
            return plugin.getMessageManager().getMessage("telegram-integration-disabled");
        }
        
        Map<String, Object> commands = plugin.getTelegramCommands();
        if (commands == null || commands.isEmpty()) {
            return plugin.getMessageManager().getMessage("telegram-commands-not-init");
        }
        
        Object commandHandler = commands.get(command);
        if (commandHandler == null) {
            return plugin.getMessageManager().getMessage("telegram-unknown-command", "command", command);
        }
        ...
        try {
            ...
            e.printStackTrace();
            return plugin.getMessageManager().getMessage("telegram-command-error", "error", e.getMessage());
        }
        
        return plugin.getMessageManager().getMessage("telegram-command-unsupported");
    }
